...
int detect_cell(unsigned char blackwhite_image[BMP_WIDTH][BMP_HEIGTH][BMP_CHANNELS])
{
  ...
  int detect_x_min;
  int detect_y_min;
  int detect_x_max = detection_area_size;
  int detect_y_max = detection_area_size;
  int flag = 0;
  for (int x = 0; x < BMP_WIDTH; x++)
  {
    for (int y = 0; y < BMP_HEIGTH; y++)
    {
      ...
      for (detect_x_min = -detection_area_size; detect_x_min < detect_x_max; detect_x_min++)
      {

        for (detect_y_min = -detection_area_size; detect_y_min < detect_y_max; detect_y_min++)
        {
          if (x + detect_x_min < 0)
          {
          ...
          if (blackwhite_image[x + detect_x_min][y + detect_y_min][0] == 255 && blackwhite_image[x + detect_x_min][y + detect_y_min][1] == 255 && blackwhite_image[x + detect_x_min][y + detect_y_min][2] == 255)
          {
            ...
            if (check_exclusion_frame(blackwhite_image, x, y) == 0)
            {
              ...
              for (int black_x = -detection_area_size; black_x < detect_x_max; black_x++)
              {

                for (int black_y = -detection_area_size; black_y < detect_y_max; black_y++)
                {
                  if (x + black_x < 0)
                  {
                    while (x + black_x != 0)
